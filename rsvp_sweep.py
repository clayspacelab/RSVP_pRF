...
subinfo = gui.Dlg(title='RSVP Sweep')
...
datafile.write('Trial_Number,Sweep_Direction,Refresh_Rate,Stim_Duration,Accuracy,Correct,Total,Mean_RT,False_Positives\n')
if len(sub_name) < 4:
...
if params['save_log']:
    ...
    stim_log.write('trial,direct,i1img,i1x,i1y,i2img,i2x,i2y,i3img,i3x,i3y,i4img,i4x,i4y,i5img,i5x,i5y,i6img,i6x,i6y,t,targ_here,targ_img,targ_slot,RT\n')
...
bore_mask_L = visual.Rect(win=win, fillColor = color, lineColor = color, size = image_h * 2)                                    # Create left bore mask
bore_mask_R = visual.Rect(win=win, fillColor = color, lineColor = color, size = image_h * 2)                                    # Create right bore mask
...
def sweep(direct, refresh_rate, trial, targ=targ, targ_rate=targ_rate, sweep_rate=sweep_rate,
    ...
    while True:
        for frame in range(1, int(sweep_rate) + 1):
            ...
            if (frame + 1) % refresh_rate == 0:
                ...
                if show_buffer:
                    if save_log:
                        stim_log.write('%i,%s,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%f,%s,%i,%i,%s\n'
                                       %(trial,direct,b_set[0],b1.pos[0],b1.pos[1],b_set[1],b2.pos[0],b2.pos[1],b_set[2],b3.pos[0],b3.pos[1],b_set[3],b4.pos[0],b4.pos[1],b_set[4],b5.pos[0],b5.pos[1],b_set[5],
                    ...
                    if save_log:
                        stim_log.write('%i,%s,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%f,%s,%i,%i,%s\n'
                                       %(trial,direct,a_set[0],a1.pos[0],a1.pos[1],a_set[1],a2.pos[0],a2.pos[1],a_set[2],a3.pos[0],a3.pos[1],a_set[3],a4.pos[0],a4.pos[1],a_set[4],a5.pos[0],a5.pos[1],a_set[5],
    ...
    datafile.write('%i,%s,%i,%f,%f,%i,%i,%f,%i\n' %(trial, direct, refresh_rate, stim_dur, accuracy, correct, total, mean_rt, false_pos))
...
for x in list(range(0, n_trials)):
    ...
    accuracy = sweep(direct = types[x % 4], refresh_rate = set_list[dur_idx], trial=trial)
    trial += 1
...
print('Run ' + str(run_number) + ' completed! \n')
win.close()
core.quit()
